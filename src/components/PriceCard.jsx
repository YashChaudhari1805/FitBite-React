...
function Card({ title, price, features, buttonText, isPopular }) {
    return (
        <>
            <div className={`w-full md:w-80 lg:w-96 bg-white shadow-md rounded-2xl border-2 flex flex-col h-full relative ${isPopular ? 'border-stone-700' : 'border-gray-100'}`}>
                ...
                <div className="p-8 flex flex-col grow">
                    ...
                    <div className="mt-auto">
                        <button className={`w-full py-3 rounded-xl font-bold transition-all duration-200 ${isPopular ? 'bg-stone-700 text-white hover:bg-black' : 'bg-gray-100 text-black hover:bg-gray-200'}`}>
                            {buttonText}
                        </button>
                    </div>
